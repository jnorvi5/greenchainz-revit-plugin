...
## 2025-05-22 - [CRITICAL] IDOR in Billing Portal
**Vulnerability:** The `/api/stripe/create-portal` endpoint accepted a `customerId` from the client request body without verifying it belonged to the authenticated user, allowing any user to access another's billing portal (IDOR).
**Learning:** Trusting client-side IDs for sensitive resources (like billing) is a common trap. Also, module-level variable initialization in Next.js API routes makes testing difficult (`process.env` mocking fails) and can lead to shared state issues.
**Prevention:**
1. Never accept resource IDs (like `customerId`) from the client if they can be derived securely from the authenticated session (e.g., via email lookup).
2. Initialize API clients (Stripe, Supabase) lazily or inside handlers to support runtime configuration and test mocking.
